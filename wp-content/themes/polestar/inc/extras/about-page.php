...
/**
 * Puro about page.
 * A basic theme about page.
 *
 * Class Puro_Extras_About_Page
 *
 * @package puro-extras
 * @license GPL 2.0 
 */
...
class Puro_Extras_About_Page {
	...
	function about_page_notice() {
		...
		<div class="updated notice is-dismissible">
			<p>
				<?php echo esc_html( sprintf( __( 'Thanks for choosing %s!', 'polestar' ), $theme->get( 'Name' ) ) ); ?>
				<?php
				printf(
					esc_html__( 'You can learn more about it %1$shere%2$s, or head straight to the %3$scustomizer%4$s to start setting it up.', 'polestar' ),
			...
			<p>
				<a href="<?php echo esc_url( admin_url( 'themes.php?page=puro-theme-about' ) ); ?>" class="button-primary">
					<?php echo esc_html( sprintf( __( 'Learn About %s', 'polestar' ), $theme->get( 'Name' ) ) ); ?>
				</a>
			</p>
		</div>
		<?php
	...
	function add_theme_page( ) {
		...
		add_theme_page(
			sprintf( __( 'About %s', 'polestar' ), $theme_name ),
			sprintf( __( 'About %s', 'polestar' ), $theme_name ),
			'edit_theme_options',
			'puro-theme-about',
	...
	function enqueue_scripts( $prefix ) {
		if ( $prefix !== 'appearance_page_puro-theme-about' ) return;

		wp_enqueue_script(
			'puro-about',
			get_template_directory_uri() . '/inc/extras/js/about.js',
			array( 'jquery' ),
			PURO_THEME_VERSION
		);
		...
		wp_enqueue_style(
			'puro-about',
			get_template_directory_uri() . '/inc/extras/css/about.css',
			array(),
			PURO_THEME_JS_PREFIX
		);
	}
			...
			case 'twitter' :
				$share_url = add_query_arg( array(
					'status' => urlencode(
						$theme->get( 'Name' ) .
						' - ' .
						__( 'Free #WordPress Theme from @PuroThemes', 'polestar' ) . ' - ' .
						$theme->get( 'ThemeURI' )
					)
	...
	function display_about_page(){
		...
		$theme = wp_get_theme( get_template() );
		$about = apply_filters( 'puro_about_page', array(
			'title' => sprintf( __( 'About %s', 'polestar' ), $theme->get( 'Name' ) ),
			'sections' => array(),
			'title_image' => false,
		...
		<div class="wrap" id="puro-about-page">
				...
				<?php if ( ! empty( $about[ 'tour_url' ] ) ) : ?>
					<li>
						<a href="<?php echo esc_url( $about[ 'tour_url' ] ) ?>" class="about-button-tour" target="_blank">
							<?php esc_html_e( 'Take a Tour', 'polestar' ) ?>
						</a>
					</li>
				<?php endif; ?>
				...
				<?php if( !empty( $about[ 'newsletter_url' ] ) ) : ?>
					<li>
						<a href="<?php echo esc_url( $about[ 'newsletter_url' ] ) ?>" class="about-button-updates" target="_blank">
							...
							<?php esc_html_e( 'Get Updates', 'polestar' ) ?>
						</a>
					</li>
				<?php endif; ?>
				...
				<?php if ( ! empty( $about[ 'documentation_url' ] ) ) : ?>
					<li>
						<a href="<?php echo esc_url( $about[ 'documentation_url' ] ) ?>" class="about-button-docs" target="_blank">
							...
							<?php esc_html_e( 'Documentation', 'polestar' ) ?>
						</a>
					</li>
				<?php endif; ?>
				...
				<?php if ( ! empty( $about[ 'review_url' ] ) ) : ?>
					<li>
						<a href="<?php echo esc_url( $about[ 'review_url' ] ) ?>" class="about-button-updates" target="_blank">
							...
							<?php esc_html_e( 'Write a Review', 'polestar' ) ?>
						</a>
					</li>
				<?php endif; ?>
				...
				<?php if ( ! empty( $about[ 'premium_url' ] ) && ! defined( 'PURO_IS_PREMIUM' ) ) : ?>
					<li class="about-highlight">
						<a href="<?php echo esc_url( $about[ 'premium_url' ] ) ?>" class="about-button-updates" target="_blank">
							...
							<?php esc_html_e( 'Upgrade to Premium', 'polestar' ) ?>
						</a>
					</li>
				<?php endif; ?>
						...
						<?php if ( empty( $about[ 'no_video' ] ) ) : ?>
							<div class="about-video-watch">
								<a href="<?php echo esc_url( $about[ 'video_url' ] ) ?>" target="_blank">
									<?php esc_html_e( 'Watch The Video', 'polestar' ) ?>
								</a>
							</div>
						<?php endif ?>
						...
						<?php if ( $theme->get( 'ThemeURI' ) ) : ?>
							<div class="about-share">
								<div class="about-share-title">
									<?php echo esc_html( sprintf( __( 'If you like %s, please share it!', 'polestar' ), $theme->get( 'Name' ) ) ) ?>
								</div>
			...
			<div class="about-puro-logo">
				<p>
					<?php echo esc_html__( 'Proudly Created By', 'polestar' ); ?>
				</p>

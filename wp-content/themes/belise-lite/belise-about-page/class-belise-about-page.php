 ...
 *            ),
 *        );
 *        Belise_About_Page::init( $config );
 *
 * @package Themeisle
 * @subpackage Admin
 * @since 1.0.0
 */
if ( ! class_exists( 'Belise_About_Page' ) ) {
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class Belise_About_Page {
		/**
		 * Define the version of the class.
		 *
		 * @var string $version The Belise_About_Page class version.
		 */
		...
		/**
		 * The single instance of Belise_About_Page
		 *
		 * @var Belise_About_Page $instance The  Belise_About_Page instance.
		 */
		private static $instance;

		/**
		 * The Main Belise_About_Page instance.
		 *
		 * We make sure that only one instance of Belise_About_Page exists in the memory at one time.
		 *
		 * @param array $config The configuration array.
		 */
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Belise_About_Page ) ) {
				self::$instance = new Belise_About_Page;
		...
		public function get_plugin_icon( $arr ) {
			...
			if ( ! empty( $arr['svg'] ) ) {
				...
				$plugin_icon_url = get_template_directory_uri() . '/belise-about-page/images/placeholder_plugin.png';
			}
			...
			if ( ! empty( $recommended_actions ) ) {
				...
				if ( ! empty( $actions ) && is_array( $actions ) ) {
					...
					foreach ( $actions as $action_key => $action_value ) {
						...
						if ( isset( $action_value['id'] ) && isset( $ti_about_page_show_required_actions[ $action_value['id'] ] ) && ( $ti_about_page_show_required_actions[ $action_value['id'] ] === false ) ) {
							$hidden = true;
						}
		...
		public function style_and_scripts( $hook_suffix ) {
			...
			wp_enqueue_style( 'belise-about-page-css', get_template_directory_uri() . '/belise-about-page/css/belise_about_page_css.css' );

			if ( 'appearance_page_' . $this->theme_slug . '-welcome' == $hook_suffix ) {

				wp_enqueue_script( 'belise-about-page-js', get_template_directory_uri() . '/belise-about-page/js/belise_about_page_scripts.js', array( 'jquery' ) );
				...
				wp_localize_script(
					'belise-about-page-js', 'tiAboutPageObject', array(
		...
		private function get_required_actions() {
			...
			$req_actions = isset( $this->config['recommended_actions'] ) ? $this->config['recommended_actions'] : array();
			$valid       = array();
			if ( ! empty( $req_actions['content'] ) ) {
				foreach ( $req_actions['content'] as $req_action ) {
					if ( ( ! isset( $req_action['check'] ) || ( isset( $req_action['check'] ) && ( $req_action['check'] == false ) ) ) && ( isset( $req_action['id'] ) && ( ! isset( $saved_actions[ $req_action['id'] ] ) || ( isset( $saved_actions[ $req_action['id'] ] ) && ( $saved_actions[ $req_action['id'] ] == false ) ) ) ) ) {
						$valid[] = $req_action;
					}
				}
			}
